...
def publish():
    ...
    _get_context(resource_id)

    encoding = _get_encoding()
    projection = _get_projection()

    #Extra parm for csv,xls
    geometry_column, geometry_type = _get_geometry_params()
...
def _get_encoding():
    ...
    if len(_encoding)==0:
        ...
        if _encoding_exists(_encoding):
            return _encoding
        else:
            abort(400, _('Bad Encoding : %s') % _encoding)


def _encoding_exists(encoding):
    try:
...
def _get_projection():
    proj_param = request.params.get('projection', u'') or '4326'
    try:
