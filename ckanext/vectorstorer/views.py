...
def vectorstorer_style_action(id, resource_id, action):
    ...
    return getattr(utils.style, action)(id, resource_id)
...
vecorstorer_views.add_url_rule(
    "/dataset/<id>/resource/<resource_id>/export/<operation>", methods=["GET", "POST"], view_func=utils.export.export,
)
vecorstorer_views.add_url_rule(
    "/api/search_epsg", methods=["GET"], view_func=utils.vector.search_epsg,
)
vecorstorer_views.add_url_rule(
    "/api/vector/publish", methods=["GET"], view_func=utils.vector.publish,
)
